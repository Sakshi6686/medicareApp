...
const BookAppointment = () => {
    ...
    const bookNow=async()=>{
        try {
            ...
            const res = await axios.post(backendUrl+"/api/user/book-appointment", { doctorId: params.doctorId,doctorInfo:doctor,userInfo:user,date:date,time:time },
              {
                headers: {
    ...
    const checkAvailability=async()=>{
        try {
            dispatch(showLoading());
            const res = await axios.post(backendUrl+"/api/user/check-booking-availability", { doctorId: params.doctorId,date:date,time:time },
              {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
  ...
  const getDoctorData = async () => {
    ...
    try {
      dispatch(showLoading());
      const res = await axios.post(backendUrl+"/api/doctor/get-doctor-info-by-id", { doctorId: params.doctorId },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
